...
export class BaileysStartupService extends ChannelStartupService {
  ...
  // Group
  // Simple in-memory controls to avoid hammering WhatsApp with groupMetadata calls on large accounts
  ...
  private async updateGroupMetadataCache(groupJid: string) {
    ...
    // If we have a fresh cache entry (<= 5 minutes), avoid requery to reduce rate-limit pressure
    const last = this.__groupMetaLastUpdate.get(groupJid) || 0;
    if (now - last < 5 * 60 * 1000) {
      ...
      // If cache miss but we recently updated, just return null to avoid immediate refetch
      return null;
    }

    // Coalesce concurrent requests for the same group
    const inflight = this.__groupMetaInFlight.get(groupJid);
    if (inflight) return inflight;

    const request = (async () => {
      try {
        // Global pacing: ensure at most ~3 requests/sec with small jitter
        const sinceGlobal = now - this.__groupMetaLastGlobal;
        const minGap = 350; // ms
        if (sinceGlobal < minGap) {
          await this.__delay(minGap - sinceGlobal + Math.floor(Math.random() * 100));
        }
        ...
        while (attempt < maxAttempts) {
          ...
          try {
            ...
            this.__groupMetaLastGlobal = Date.now();
            this.__groupMetaLastUpdate.set(groupJid, Date.now());
            ...
            lastErr = error;
            const isRateLimit = (error as any)?.data === 429 || (error as any)?.message?.includes?.('rate-overlimit');
            if (isRateLimit && attempt < maxAttempts) {
              const backoff = 1000 * attempt + Math.floor(Math.random() * 250);
              this.logger.warn(`groupMetadata rate-limited for ${groupJid}. retry ${attempt}/${maxAttempts - 1} in ${backoff}ms`);
              await this.__delay(backoff);
              continue;
            }
            throw error;
          }
        ...
        this.__groupMetaInFlight.delete(groupJid);
      }
    })();

    this.__groupMetaInFlight.set(groupJid, request);
    return request;
  }
